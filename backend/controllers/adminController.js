...
exports.getAdminDashboard = (req, res) => {
  ...
  const salesByDay     = db.prepare(`SELECT DATE(created_at) as date,COUNT(*) as orders,SUM(final_amount) as revenue FROM orders WHERE status!='cancelled' AND created_at>=DATE('now','-7 days') GROUP BY DATE(created_at) ORDER BY date`).all();
  const topProducts    = db.prepare(`SELECT p.name,SUM(oi.quantity) as total_sold,SUM(oi.total_price) as revenue FROM order_items oi JOIN products p ON oi.product_id=p.id GROUP BY p.id ORDER BY total_sold DESC LIMIT 5`).all();
  const usersByRole    = db.prepare(`SELECT role,COUNT(*) as count FROM users GROUP BY role`).all();

  res.json({ success:true, data:{ summary:{ totalUsers,totalOrders,totalRevenue,pendingOrders,pendingVendors,pendingDrivers,activeDrivers }, recentOrders,ordersByStatus,salesByDay,topProducts,usersByRole } });
};
